...
Optionally provide a Hugging Face access token using `+"`"+`--access-token-hugging-face`+"`"+`.`,
		Writer,
		aliasOpt("c"),
	...
	AddStringFlag(cmdCreate, doctl.ArgDedicatedInferenceAccessTokenHuggingFace, "", "", "Hugging Face token for accessing gated models (optional)")
	cmdCreate.Example = `The following example creates a dedicated inference endpoint using a spec file: doctl dedicated-inference create --spec spec.yaml --access-token-hugging-face "hf_mytoken"`

	return cmd
}

// readDedicatedInferenceSpec reads and parses a dedicated inference spec from a file path or stdin.
func readDedicatedInferenceSpec(stdin io.Reader, path string) (*godo.DedicatedInferenceSpec, error) {
	...
	var spec godo.DedicatedInferenceSpec
...
func RunDedicatedInferenceCreate(c *CmdConfig) error {
	...
	hfToken, _ := c.Doit.GetString(c.NS, doctl.ArgDedicatedInferenceAccessTokenHuggingFace)
	if hfToken != "" {
		req.AccessTokens = &godo.DedicatedInferenceAccessTokens{
