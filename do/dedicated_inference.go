...
type DedicatedInferenceService interface {
	Create(req *godo.DedicatedInferenceCreateRequest) (*DedicatedInference, error)
}
...
func (s *dedicatedInferenceService) Create(req *godo.DedicatedInferenceCreateRequest) (*DedicatedInference, error) {
	d, _, err := s.client.DedicatedInferences.Create(context.TODO(), req)
	if err != nil {
		return nil, err
	}
